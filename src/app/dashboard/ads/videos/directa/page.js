...
import { VideoFrameworkPicker } from "@/components/ads/video-framework-picker";
...
import {
  ...
  getDurationById,
  getVideoFrameworkById,
} from "@/lib/video-frameworks";
import {
  buildStoryboard,
  formatTimestamp,
  regenerateScene,
...
const STEPS = [
  ...
  { id: "framework", label: "Estructura ganadora" },
  { id: "storyboard", label: "Storyboard + voz-over" },
];
...
export default function VideosDirectaPage() {
  ...
  function handleRegenerateScene(sceneId) {
    setStoryboard((prev) => (prev ? regenerateScene(prev, sceneId, angle) : prev));
  }

  function handleRegenerateAll() {
    setGenerating(true);
    setTimeout(() => {
      const next = buildStoryboard({ frameworkId, durationId, angle });
      setStoryboard(next);
      setActiveSceneId(next?.scenes?.[0]?.id ?? null);
    }, 100);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Ads · Vídeos · Directa"
        title="Storyboard listo para grabar"
        description="Convierte el ángulo del Brief en un guion + storyboard escena a escena. Voz en off, on-screen text y b-roll listos para grabar y montar."
        badges={[
          { label: "Hook · Reveal · CTA" },
          { label: "Pre-fill desde Brief", variant: "violet" },
          {
            label: duration ? `Target · ${duration.label}` : "Target · 30s",
            variant: "success",
          },
        ]}
        action={
              ...
              <div className="mb-2 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Ajusta el ritmo del storyboard.
                  </p>
                </div>
                <Badge variant="violet">{VIDEO_DURATIONS.length} formatos</Badge>
              </div>
            ...
            <header className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Cada estructura tiene sus beats y tipos de toma. Aplicamos tu
                  ángulo encima.
                </p>
              </div>
        ...
        {stage === "storyboard" && storyboard && (
          ...
          >
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    {" "}
                    {storyboard.scenes.length} escenas
                  </p>
                </div>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <StatTile label="Duración" value={`${storyboard.totalSeconds}s`} />
              <StatTile label="Escenas" value={storyboard.scenes.length} />
              <StatTile
                label="Estructura"
      ...
      <GenerationOverlay
        open={generating}
        title="Generando storyboard"
        subtitle={
          framework
            ? `Aplicando ${framework.name} en ${duration?.seconds || 30}s`
            : "Aplicando estructura ganadora con tu Brief"
        }
